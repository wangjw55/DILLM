...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            with open('tasks/R2R/data/R2R_%s_subgoals.json' % split) as f:
...
def read_obj_features(feature_store):
    ...
    with open(feature_store, "r") as tsv_in_file:     # Open the tsv file.
        ...
        for item in reader:
            long_id = item['scanId'] + "_" + item['viewpointId']
            features[long_id] = np.frombuffer(base64.decodestring(item['features'].encode('ascii')),
                                                   dtype=np.float32).reshape((views, -1))   # Feature of long_id is (12, 512)

    print("Finish Loading the object feature from %s in %0.4f seconds" % (feature_store, time.time() - start))
    return features
